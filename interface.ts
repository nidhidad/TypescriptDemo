export interface IUser{
    name: string;
    age?: number; // to make a property optional add ?
    email: string;
    id: number;
}

let user : IUser = { name: 'John Doe', email: '', id: 1 };
console.log(user); // { name: 'John Doe', email: '', id: 1 }

// this is known as object destructuring
let {name: userName, email: userLogin} : IUser = { name: 'John Doe', email: '', id: 1, age: 30 };
...
let employee: IEmployee = { name: 'John Doe', email: '', id: 1, salary: 1000 };
console.log(employee); // { name: 'John Doe', email: '', id: 1, salary: 1000 }

// export keyword is used to export the interface so that it can be imported in other files
